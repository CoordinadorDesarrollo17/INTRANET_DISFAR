                ...
                using (SqlTransaction transaction = cn.BeginTransaction())
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_MantenimientoSolicitudTraslado", cn, transaction))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@OperarioResponsableSAP", obj.OperarioResponsable);
                            ...
                            if (obj.Detalle != null)
                            {
                                foreach (var detalle in obj.Detalle)
                                {
                                    table.Rows.Add(
                                        ...
                                        detalle.ItemName,
                                        detalle.BatchNum,
                                        detalle.CantidadTotalPorSKUyLote,
                                        detalle.AlmacenOrigen,
                                        detalle.AlmacenDestino,
                                        "PENDIENTE"
                                    );
                                }
                            }
