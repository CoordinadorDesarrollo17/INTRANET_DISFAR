...
namespace Capa_Datos.Rutas_DAO.TablasSql
{
	public class ORRU_D
	{
			...
			using (SqlConnection cn = new SqlConnection(uti.cadSql))
			{
				try
				{
					...
					try
					{
						...
						cmd.Parameters.AddWithValue("@TiempoPac", o.TiempoPac);
		...
		public int EditarOrdenDeRuta(ORRU_E o)
		{
			int status = -1;
			SqlConnection cn = new SqlConnection(uti.cadSql);
			try
		...
		public List<ORRU_E.RptRutas> ReporteHojasCargo(ORRU_E o)
		{
			...
			try
			{
				...
				cmd.Parameters.AddWithValue("@FecConFin", o.FecConFin);
				cmd.Parameters.AddWithValue("@CardCode", o.CardCode);
				cmd.Parameters.AddWithValue("@TipoRuta", "AC");
				cmd.Parameters.AddWithValue("@TransDesc", o.TransDesc);
				cmd.Parameters.AddWithValue("@Placa", o.Placa);
				cmd.Parameters.AddWithValue("@MontoTotalIni", o.MontoTotalIni);
				cmd.Parameters.AddWithValue("@MontoTotalFin", o.MontoTotalFin);
		...
		{
			List<ORRU_E.RptRutasDet> lista = new List<ORRU_E.RptRutasDet>();
			SqlConnection cn = new SqlConnection(uti.cadSql);

			try
			{
				SqlCommand cmd = new SqlCommand("al.RptHojaCargoDetallado", cn)
				{
					CommandType = CommandType.StoredProcedure
				};
				...
				SqlDataReader dr = cmd.ExecuteReader();
				ORRU_E.RptRutasDet or = new ORRU_E.RptRutasDet();
				...
				while (dr.Read())
				{
					...
					if (!dr.IsDBNull(34)) { or.GastoEnvio = dr.GetDecimal(34); }
					if (!dr.IsDBNull(35)) { or.TipoVenta = dr.GetString(35); }
					if (!dr.IsDBNull(36)) { or.LineaPeso = dr.GetInt32(36); }
					if (!dr.IsDBNull(37)) { or.Peso = dr.GetDecimal(37); }
					if (!dr.IsDBNull(38)) { or.UniMed = dr.GetString(38); }
					if (!dr.IsDBNull(39)) { or.PrecioEnv = dr.GetDecimal(39); }
					lista.Add(or);
				}
				cn.Close();
				dr.Close();
